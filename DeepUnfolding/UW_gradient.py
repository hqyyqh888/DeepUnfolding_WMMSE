...
def U_gradient_lastlayer(Nr,Nt,d,Pt,sigma,User,H,W,U):
    ...
    check_gradient_U=[]
    for k in range(User):
        check_gradient_U.append(np.matrix(np.zeros((Nr,d),dtype=np.complex)))
    ...
    for k in range(User): 
        for i in range(Nr):
            for j in range(d):
                U_hat=U.copy()
                U_delta=np.matrix(np.zeros((Nr,d),dtype=np.complex)) 
                U_delta[i,j]=delta
                U_hat[k]=U[k]+U_delta           
                check_gradient_U[k][i,j]=( obj_func.objective_func_UW(Nr,Nt,d,Pt,sigma,User,H,W,U_hat) - obj_func.objective_func_UW(Nr,Nt,d,Pt,sigma,User,H,W,U) )/(2*delta)          
    ...
    for k in range(User): 
        for i in range(Nr):
            for j in range(d):
                U_hat=U.copy()
                U_delta=np.matrix(np.zeros((Nr,d),dtype=np.complex)) 
                U_delta[i,j]=delta*1j
                U_hat[k]=U[k]+U_delta           
                check_gradient_U[k][i,j] += 1j*( obj_func.objective_func_UW(Nr,Nt,d,Pt,sigma,User,H,W,U_hat) - obj_func.objective_func_UW(Nr,Nt,d,Pt,sigma,User,H,W,U) )/(2*delta)           
             
    return Gradient_U #, check_gradient_U        



######################## Calculate the gradient of W in the last layer 
def W_gradient_lastlayer(Nr,Nt,d,Pt,sigma,User,H,W,U):  
#    global User, Layer, Nt, Nr, d, Pt, sigma
#    global X_U, X_W, X_V, Y_U, Y_W, Y_V, Z_U, Z_W, Z_V, O_U, O_V, H, U, W, V
    ...
    check_gradient_W=[]
    for k in range(User):
        check_gradient_W.append(np.matrix(np.zeros((d,d),dtype=np.complex)))
    ...
    for k in range(User): 
        for i in range(d):
            for j in range(d):
                W_hat=W.copy()
                W_delta=np.matrix(np.zeros((d,d),dtype=np.complex)) 
                W_delta[i,j]=delta
                W_hat[k]=W[k]+W_delta           
                check_gradient_W[k][i,j]=( obj_func.objective_func_UW(Nr,Nt,d,Pt,sigma,User,H,W_hat,U) - obj_func.objective_func_UW(Nr,Nt,d,Pt,sigma,User,H,W,U) )/(2*delta)          
    ...
    for k in range(User): 
        for i in range(d):
            for j in range(d):
                W_hat=W.copy()
                W_delta=np.matrix(np.zeros((d,d),dtype=np.complex)) 
                W_delta[i,j]=delta*1j
                W_hat[k]=W[k]+W_delta           
                check_gradient_W[k][i,j] += 1j*( obj_func.objective_func_UW(Nr,Nt,d,Pt,sigma,User,H,W_hat,U) - obj_func.objective_func_UW(Nr,Nt,d,Pt,sigma,User,H,W,U) )/(2*delta)           
                
    return Gradient_W #, check_gradient_W
